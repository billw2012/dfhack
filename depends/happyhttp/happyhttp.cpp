...
//
//---------------------------------------------------------------------
happyhttp_exception::happyhttp_exception( const char* fmt, ... )
{
    va_list ap;
    va_start( ap,fmt);
...
void Connection::connect()
{
    ...
    if( !addr )
        throw happyhttp_exception( "Invalid network address" );
    ...
    // TODO: fix the type of SOCKET correctly
...
{
    if( m_State != IDLE )
        throw happyhttp_exception( "Request already issued" );
...
void Connection::putheader( const char* header, const char* value )
{
    if( m_State != REQ_STARTED )
        throw happyhttp_exception( "putheader() failed" );
...
{
    if( m_State != REQ_STARTED )
        throw happyhttp_exception( "Cannot send header" );
    m_State = IDLE;
...
void Response::notifyconnectionclosed()
{
    ...
    {
        throw happyhttp_exception( "Connection closed unexpectedly" );
    }
}
...
void Response::ProcessStatusLine( std::string const& line )
{
    ...
    if( m_Status < 100 || m_Status > 999 )
        throw happyhttp_exception( "BadStatusLine (%s)", line.c_str() );
/*
...
*/
    if( m_VersionString == "HTTP:/1.0" )
        ...
        throw happyhttp_exception( "UnknownProtocol (%s)", m_VersionString.c_str() );
